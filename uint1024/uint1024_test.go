...
// TestUint1024Creation tests basic creation and initialization of Uint1024
func TestUint1024Creation(t *testing.T) {
	// Test New
	u1 := New(42)
	if u1.words[0] != 42 {
...
// TestLargeNumbers tests operations with large numbers
func TestLargeNumbers(t *testing.T) {
	// Test with maximum uint64 value
	maxU64 := New(^uint64(0))
	one := New(1)

	// Addition should not overflow for this case
	sum := maxU64.Add(one)

	// The result should be 2^64
	expected := ZERO.Clone()
	expected.words[1] = 1 // Set bit 64
	if !sum.Equal(expected) {
		t.Error("Max uint64 + 1 should equal 2^64")
	}
}
...
func TestDivision(t *testing.T) {
	...
	if !quotient.Equal(expectedQ) {
		t.Errorf("100 / 10 quotient: got %s, want %s", quotient.String(), expectedQ.String())
	}
	...
	expectedR := New(0)
	if !remainder.Equal(expectedR) {
		t.Errorf("100 %% 10 remainder: got %s, want %s", remainder.String(), expectedR.String())
	}
